...
bool ConfigureParser::init()
{
  ...
  for (int i = 0; i < stream_size; i++) {
    int height = node_handler_->declare_parameter(stream_name[i] + ".height", 1080);

    int width = node_handler_->declare_parameter(stream_name[i] + ".width", 1920);

    int fps = node_handler_->declare_parameter(stream_name[i] + ".fps", 30);
